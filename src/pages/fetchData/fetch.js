...
import { FETCH_ALL_DISCUSSIONS, FETCH_ANSWERED_USERNAMES } from '../../lib/resolvers/query';

export const fetchData = async (client) => {
    const [answeredUsernamesData, allDiscussionsData] = await Promise.all([
      client.query({ query: FETCH_ANSWERED_USERNAMES }),
      client.query({ query: FETCH_ALL_DISCUSSIONS })
    ]);

  let userPostsCount = allDiscussionsData.data.repository.discussions.edges.reduce((acc, edge) => {
  ...
  let userAnswersCount = answeredUsernamesData.data.repository.discussions.edges.reduce((acc, edge) => {
